...
public class XTMarketDataService extends XTMarketDataServiceRaw implements MarketDataService {
  ...
  public XTMarketDataService(XTExchange exchange, ResilienceRegistries resilienceRegistries) {
    super(exchange, resilienceRegistries);
  }

  @Override
  public Ticker getTicker(Instrument instrument, Object... args) throws IOException {
    List<XTTicker> tickers = super.getTickers(XTAdapters.adaptInstrument(instrument), null);
  ...
  public List<Ticker> getTickers(Params params) throws IOException {
    if (params == null) {
      return XTAdapters.adaptTickers(super.getTickers(null, null));
    }
    if (params instanceof InstrumentsParams) {
      ...
      String symbols = is.stream().map(i -> i.getBase() + "_" + i.getCounter())
          .reduce((a, b) -> a + "," + b).get();
      List<XTTicker> tickers = super.getTickers(null, symbols);
